...
contract ApeRouter is IApeRouter02 {
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = ApeLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "ApeRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = ApeLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function checkPairAddress(address tokenA, address tokenB) view external returns (address) {
        return ApeLibrary.pairFor(factory, tokenA, tokenB);
    }
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = IApePair(pair).burn(to);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? ApeLibrary.pairFor(factory, output, path[i + 2])
                : _to;
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            ApeLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
